...
Multiscale Albedo is a mode of the same Height tool. It uses linear Rec.709 luminance with three Gaussian-like detail bands (Coarse, Medium, Fine). Remove broad lighting suppresses slow illumination changes; turn it off to retain broad brightness shapes. It is an artistic fallback: pigment and shadows are not physical surface depth. Changing modes selects the corresponding detected normal/basemap role; a manual source stays assigned.

DeepBump is the third Height mode. It predicts normals from albedo with the optional [HugoTini/DeepBump model](https://github.com/HugoTini/DeepBump), then uses the same normal integration solver. Inference runs locally on CPU in a cancellable Python worker; source images are never uploaded. The model receives sRGB grayscale with overlapping 256-pixel tiles, blended with Hann windows, and returns normalized normals.

For setup, choose a Python 3.10–3.13 executable with pip and click **Install DeepBump**. Setup installs NumPy 2.2.6 and ONNX Runtime 1.22.1 into a fresh private directory under `Library/TexturePackEditor/DeepBump`, downloads the model, verifies its SHA-256, and runs an inference self-test. The active installation changes only after success. A failed repair leaves the previous installation usable; old installations remain available to jobs already running. Deleting this Library directory removes the optional backend and requires setup again. Python and model paths are machine-local, outside recipe assets and version control. Click any error box to copy its full message; a notification confirms the copy.

Model revision: `fad19ba87daed12b1d0410a57e74f3d79e82f78d`; SHA-256: `3a2ababc5fa652b19040a0f3d639b71826368e52ad805cf70ffb0e7c96a54d67`. The model is distributed by DeepBump under GPL-3.0; setup retains its upstream license alongside the downloaded model. The adapter uses its own tiling implementation. No model weights or Python environment are bundled in this repository.

`Tests~/HeightChecks.cs` contains standalone analytic algorithm checks. `HeightIntegrationChecks.cs` runs inside an isolated Unity project and checks all three modes, preview/export equality, crop consistency, terrain assignment, source preservation and cancellation (DeepBump must be installed). `DeepBumpSetupChecks.cs` checks failed setup preservation and successful installation switching. Copy test sources and `Editor/*` into the isolated project's `Assets/Editor` and use their `Run` entry points with Unity batchmode; these fixtures create/delete only their `Assets/HeightFixtures` test assets. Do not run asset fixtures in a production project.

Center, Strength and blend edits reuse the solved height cache before GPU source capture. Each preview job pins its cached map so cache eviction cannot invalidate an active job. Node thumbnails and the main preview reuse readable Texture2D objects when dimensions are unchanged, avoiding repeated native texture destruction during slider edits. `HeightSliderChecks.Run` stress-tests 200 Center/Strength updates across editor frames and verifies both cache reuse and stable preview texture identity.

The settings gear now contains the safe output suffix. Recipe edits participate in Unity Undo/Redo and invalidate the preview when restored. DeepBump keeps its setup Python field available after installation, validates the interpreter/dependencies/model with an asynchronous inference self-test, and offers Check, Repair and Clean unused installations. Cleanup only removes installer-owned GUID folders and preserves active installations and runtimes leased by captured jobs.

Pending node previews show Unity's animated WaitSpin icon after 0.2 seconds, including collapsed-node labels. Timers survive repeated edits until a current preview arrives, and clear on completion or failure. `RecipeUndoChecks`, `DeepBumpRecoveryChecks` and `PreviewSpinnerChecks` cover saved recipe Undo/Redo, runtime health/cleanup, and spinner timing/icon availability in the isolated Unity test project.
